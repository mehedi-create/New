...
const BASE = config.apiBaseUrl.replace(/\/+$/, '') // trim trailing slash
...
// Existing
export const getUserBootstrap = (address: string) => api.get(`/api/users/${address}`)
...
export const getDashboardData = (walletAddress: string) => api.get(`/api/dashboard/${walletAddress}`)
...
export const createNotice = (payload: NoticePayload) => api.post('/api/notices', payload)
export const updateNotice = (id: number, payload: NoticePayload) => api.patch(`/api/notices/${id}`, payload)
